...
public class MultiSplitLayout implements LayoutManager
{
  ...
  public Node getNodeForName( String name )
  {
    ...
    else if ( model instanceof Leaf && name.equals(((Leaf)model).getName())) {
      return model;
    }
    return null;
  }
        ...
        if (removeDividers) {
          while ( s.getChildren().size() < 2 ) {
            ...
            if ( s.getChildren().size() == 1 ) {
              ...
              p.replace( s, next );
              //next.setParent( p ); /// KHK: unnecessary, replace sets parents
            }
            else {
              p.remove( s );
            }
  ...
  public static class Split extends Node {
    ...
    public void replace( Node target, Node replacement ) { 
      ...
      target.setParent( null ); // KHK: set parent to null instead of this
    }
